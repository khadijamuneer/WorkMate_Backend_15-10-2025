  ...
  return (
    <svg width={size} height={size}>
      ...
      <text x="50%" y="50%" dominantBaseline="middle" textAnchor="middle"
        fontSize={size < 70 ? "11" : "16"} fontWeight="800" fill={c}>{Math.round(score)}</text>
    </svg>
  );
};
...
const sc = (s) => s >= 70 ? "#10b981" : s >= 45 ? "#3b4bff" : "#f59e0b";
...
const InterviewDetail = () => {
  ...
  useEffect(() => {
    if (data) return; // already have it from state
    fetch(`http://127.0.0.1:8000/video-interview/history/${id}`, {
  ...
  const overallLabel = overall_score >= 75 ? "Excellent" : overall_score >= 60 ? "Good" :
                       overall_score >= 45 ? "Fair" : "Needs Work";
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "2rem" }}>
            <div>
              ...
              <p style={{ color: "#9ca3af", fontSize: "0.82rem", margin: "4px 0 0" }}>
                {fmtDate(createdAt)}
                {speech?.duration_sec ? ` · ${Math.round(speech.duration_sec / 60)}m ${Math.round(speech.duration_sec % 60)}s` : ""}
              </p>
            </div>
          ...
          <div style={{ background: "#fff", borderRadius: "20px", border: "1px solid #ebebf0",
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "1rem", marginTop: "1rem" }}>
                {[["Voice", voice?.score], ["Eye Contact", eye_contact?.score],
                  ["Posture", posture?.score], ["Content", content_avg]
                ].map(([label, score]) => (
                  <div key={label}>
                      ...
                      <>
                        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "0.5rem", marginBottom: "0.75rem" }}>
                          {[["Relevance", q.relevance], ["Clarity", q.clarity],
                            ["Depth", q.depth], ["Structure", q.structure]].map(([l, v]) => (
                            <div key={l}>
